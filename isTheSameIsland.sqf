if (worldName != "Chernarus") exitWith {true};
...
if (_posSitio1 distance getMarkerPos "isla" <= 5500) then
	...
	if (_posSitio1 distance getMarkerPos "isla_1" <= 2000) then
		...
		{
		if (_posSitio1 distance getMarkerPos "isla_2" <= 2500) then
			{
			if (_posSitio2 distance getMarkerPos "isla_2" <= 2500) then {_return = true}
			}
		else
			{
